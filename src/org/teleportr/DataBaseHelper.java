...
class DataBaseHelper extends SQLiteOpenHelper {
	...
	static final String INSERT_PLACE =
	...
	public long insertPlace(ContentValues cv) {
		...
		if (place_id == -1) { // insert has been ignored as place already exists
			Cursor c = getReadableDatabase().rawQuery("SELECT _id from places" +
				" WHERE name=?", new String[] { cv.getAsString("name") });
			c.moveToFirst();
	...
	static final String INSERT_KEY =
			"INSERT OR IGNORE INTO place_keys" +
			" ('place_id', 'key', 'value')" +
			" VALUES (?, ?, ?);";
	...
	static final String INSERT_RIDE =
	...
	static final String SELECT_RIDES = 
			"SELECT rides._id," +
					" \"from\".name, \"from\".address," +
					" \"to\".name, \"to\".address, dep" +
			" FROM 'rides'" +
